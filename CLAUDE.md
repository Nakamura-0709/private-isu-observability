...
書籍のハンズオンに沿った変更のPRは、以下の構成にする。

- `## 概要`: 対応する書籍の節番号と、何を・なぜ追加/変更したかを数文で書く。
- `## 書籍との差分`: 実装で書籍から変えた点（1Passwordエージェント対応など）を書く。差分がなければ省略してよい。
- `## 動作確認`: 確認方針・確認済み内容を書く。AWS課金を避けるため複数節をまとめて確認する場合はその旨を記す。

本文は1文ごとに改行する。
「変更内容」「変更ファイル」の列挙は書かない（Files changed で確認できるため）。
